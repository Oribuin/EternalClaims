...
public class TrustCommand extends RoseCommand {

    public TrustCommand(RosePlugin rosePlugin, RoseCommandWrapper parent) {
    ...
    public void execute(CommandContext context, Player target, @Optional String allClaims) {
        ...
        if (claims.isEmpty()) {
            player.sendMessage(Component.text("There are no claims to trust this player in"));
            return;
        }

        CompletableFuture.runAsync(() -> claims.stream().filter(claim -> claim.trust(player)).forEach(manager::save))
                .thenRun(() ->
                        player.sendMessage(Component.text("Trusted player " + target.getName() + " in all claims"))
                );
    }
